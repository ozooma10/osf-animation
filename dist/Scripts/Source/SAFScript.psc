...
Bool Function StopAnimation(Actor akActor) Global
    If akActor == None
        return false
    EndIf
    ; Release any standalone lock this shim engaged directly (LockActorForAnimation).
    ; Idempotent + ref-count-guarded, so it's a no-op when the player wasn't locked; for a
    ; scene, the runtime's own lock is released by the stop below.
    ...
    If OSF.StopSceneForActor(akActor)
        SAFLog("StopAnimation actor=" + akActor + " (scene)")
        return true
    EndIf
...
Bool Function PlayScene(Actor akActor1, Actor akActor2, String asAnim1, String asAnim2, Float fSpeed = 1.0) Global
    ...
    return OSFAdvanced.StartSceneFiles(actors, files, opts) != 0
EndFunction
...
Bool Function PlaySceneSeparate(Actor akActor1, Actor akActor2, String asAnim1, String asAnim2, Float fSpeed = 1.0) Global
    ...
    ; OSFAdvanced.StartSceneFiles already refreshes capsules / anchors actor2; same path.
...
; SAF's SyncGraphs frame-locked a group on one shared clock AND co-located them.
; OSF.Sync(abAnchor=true) reproduces that: it promotes the actors' independently-played
; solo graphs into ONE anchored scene at akTargets[0], so each paired clip's baked root
; offset arranges the actors about a shared origin+heading. This is what aligns mods that
; play each actor solo then call SyncGraphs (SnuSnu) — without it every actor anchors at
; its own world position (the "actors stand apart / mis-aligned" symptom).
...
; SyncGraphs anchors the group into a scene, so the actor is now a scene participant.
; Tear that scene down so the actor returns to a plain solo state — required before the
; caller re-plays a new clip (OSF.Play refuses scene participants). No-op if not synced.
...
; --- Sequences -- map onto OSFAdvanced.PlaySequence (solo multi-phase) ---

; Each path is one phase; phases play once then auto-advance, bLoop restarts the
; whole sequence after the last (OSFAdvanced.PlaySequence).
Function StartSequence(Actor akActor, String[] asPaths, Bool bLoop) Global
    ...
    While i < n
        ...
        i += 1
    EndWhile
    OSFAdvanced.PlaySequence(akActor, files, loops, blends, bLoop)
EndFunction
...
Actor[] Function GetSelectionBuffer() Global
    ...
    ; An uninitialized array is None == empty (Length 0), consistent with GetSelectionBufferSize().
    ; Papyrus can't `new` a zero-length array, so a None array is the canonical empty here.
    Actor[] empty
    return empty
EndFunction
