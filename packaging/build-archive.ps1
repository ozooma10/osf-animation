...
.DESCRIPTION
  Flow:
    1. (unless -SkipBuild) clean-configure xmake with profiling disabled, then build
...
# The scene-browser view (generated by the OSF UI CLI — the xmake run above regenerates it
...
# in OSF UI. Without it the browser UI simply doesn't exist. Copy the complete generated
# views tree because each view's HTML references the CLI's namespace-root assets.
$viewsSrc = Join-Path $repo 'build\osfui\SFSE\Plugins\OSFUI\views'
$viewSrc = Join-Path $viewsSrc 'osf.animation\browser'
...
$viewsDst = "$core\SFSE\Plugins\OSFUI\views"
New-Item -ItemType Directory -Force -Path $viewsDst | Out-Null
Copy-Item "$viewsSrc\*" $viewsDst -Recurse
...
Get-ChildItem $viewsDst -Recurse -Filter '*.map' | Remove-Item -Force
...
# Optional browser content: self-contained player.emote.* scenes.
...
Write-Host '    and Immersion Emotes are the opt-in steps). Requires SFSE + OSF UI.'
