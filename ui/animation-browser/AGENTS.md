...
- Production output is generated by `@osfui/cli`. Avoid unsupported browser APIs, dynamic imports,
  ...
  `RTCPeerConnection`, and output must stay ES2018. ⚠ `osfui check` scans a view's *source
  directory*, which for this project is only the `src/views/osf.animation/browser/` entry stub, so
  the app itself is NOT covered — treat these as hand-held invariants, not enforced ones.
