...
function Header({ state, commands }: { state: BrowserState; commands: BrowserCommands }) {
  return <><div class="slate"><div class="slate-cell slate-brand"><div class="brand-lockup"><div class="brand-emblem"><span/></div><div class="brand-meta"><div class="brand-title">Animation Browser</div><div class="brand-sub"><Status state={state} commands={commands}/></div></div></div><div class="brand-tools"><button class="iconbtn" type="button" title="Refresh catalog" onClick={commands.refresh}>⟳</button><button class={`iconbtn settings-btn ${state.settingsOpen ? "on" : ""}`} type="button" title="Browser settings" aria-label="Browser settings" aria-pressed={state.settingsOpen} onClick={() => commands.toggleSettings()}>⚙</button><span class="brand-tool-sep"/><button class="iconbtn" type="button" title="Minimize — watch the scene" onClick={() => commands.setMinimized(true)}><svg class="chev-ico" width="10" height="9" viewBox="0 0 10 9" aria-hidden="true"><path d="M1 1l4 3.8L9 1"/><path d="M1.2 8h7.6"/></svg></button></div></div><div class="slate-cell slate-take"><RunningSummary state={state} commands={commands}/></div></div><div class="stripe"><span/><span/><span/></div></>;
}
