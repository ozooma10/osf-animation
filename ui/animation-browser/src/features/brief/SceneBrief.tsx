...
function AnimationList({ state, scene, canPlay, focusStage, commands }: { state: BrowserState; scene: SceneModel; canPlay: boolean; focusStage?: number | null; commands: BrowserCommands }) {
  ...
  const focused = focusStage == null ? null : scene.stages.find((stage) => stage.index === focusStage);
  ...
  return <div class="info-box"><div class="lbl">ANIMATIONS · {shown.length}{folded ? ` OF ${scene.stages.length}` : ""}</div><div class="anim-list">
    ...
    {focusStage == null && !!clean.length && !!noise.length && <button class={`reveal anim-fold ${state.briefFullAnims ? "on" : ""}`} onClick={commands.toggleBriefAnimations}>{folded ? `+ ${noise.length} transition${noise.length === 1 ? "" : "s"} & layers ▸` : "poses & loops only"}</button>}
  </div></div>;
}
...
export function SceneBrief({ state, commands }: { state: BrowserState; commands: BrowserCommands }) {
  ...
  return <>
    ...
    <div class="brief-foot"><Overrides state={state} commands={commands}/><div class="launch-stack">
      ...
      {canPlay ? <button class="launch-btn go" onClick={() => commands.launch(focusedStage?.index, !!focusedStage)}>▶ {focusedStage ? "Play Animation" : emote ? "Play Emote" : "Launch Scene"}</button> : <button class="launch-btn blocked" disabled>{!state.ready ? "Engine Offline" : `Blocked · ${evaluation.gaps} gap${evaluation.gaps > 1 ? "s" : ""}`}</button>}
