...
namespace OSF::Serialization
{
	...
	{
	public:
		// Loads a .glb/.gltf file (transparently gunzipping if needed), 
		// builds an ozz skeleton from its node hierarchy and compiles the selected animation (by name, or by index if a_animId is numeric; 
		// empty selects the first animation) into an ozz runtime animation.
		//
		// Successful results are memoized in a process-wide cache keyed on (normalized path, animId): 
		// the ozz data is immutable behind shared_ptr<const>, 
		// so a replay or a multi-stage preload of the same file costs one gunzip+parse+build per session instead of one per use.
		// The cache holds strong references (bounded by the distinct clips actually played in a session) — ClearCache() drops it.
