...
int main()
{
...

	{
		...
		if (Check(wem.size() == 72, "PCM WEM includes its fixed header and stereo payload")) {
	...
	}
	{
		...
		if (Check(wem.size() == 64, "empty PCM WEM is a header-only file")) {
			Check(ReadU32(wem, 60) == 0, "empty PCM WEM has no sample payload");
	...
	return Finish("Sound registry and PCM/WEM");
}
